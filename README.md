...
- `.github/actions/sast-scan/` — first shared action
- `.github/actions/secret-scan/` — second shared action
- `.github/workflows/ci.yml` — consumer CI workflow

A separate flag controls each action's version, so the two roll out independently.
...
3. Each subsequent step pins its `uses:` ref by concatenating the action's name prefix with the LD-returned value: `@sast-scan-${{ steps.ld.outputs.sast-scan-version }}` resolves to `sast-scan-v1`, `sast-scan-v2`, or `sast-scan-stable` depending on what LD returns. Each action's refs are versioned and advanced independently. (FN2)
